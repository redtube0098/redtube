// api/admin.js
//
// MERGED ADMIN API — this one file replaces what used to be 5 separate
...
// this app was sitting at exactly 12 (see the old comment that used to be
// at the top of api/bot.js). Every file directly under /api (that doesn't
// start with "_") becomes its own function, so 5 admin files = 5 of those
// 12 slots. Merging them into ONE file with an internal ?resource=
// dispatcher drops the total from 12 down to 8, freeing 4 slots for
// whatever gets added next.
//
// HOW ROUTING WORKS NOW: instead of separate URLs like
// "/api/admin/withdraws" and "/api/admin/tasks", every admin call now
// hits this single "/api/admin" route with a `resource` query param
// telling it which of the 5 sections to run — e.g.
// "/api/admin?resource=withdraws". The frontend (public/admin.js) was
// updated in exactly ONE place (its shared api() helper) to rewrite the
// old-style paths into this new shape automatically — none of its ~27
// individual call sites had to change.
//
